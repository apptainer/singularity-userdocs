...
You obviously can’t have them at separate times. You would have to source
some custom environment file (that you make on your own) and it gets
confusing with issues of using shell and sourcing for the container. We don’t know who
...
the path, and its ``lib`` folder added to the ``LD_LIBRARY_PATH`` . This means that anything you drop in
...
app’s (foo) data and metadata. They look like the following:
...
the commands are all done in context of that base. The ``bin`` and ``lib`` directories are
...
-  locations for input, output, and general data are exposed. It’s up to
   ...
   application will look for inputs and outputs in it’s specific folder.
...
This design means that we can have apps interact
...
runscript is associated with a particular action of the container, users
can better encapsulate how dependencies relate to each step in a scientific
work-flow. Making containers can be challenging. When a scientist starts to
...
It also uses the ``fortune`` program to produce random "fortunes" and the ``lolcat`` application that adds rainbow color to an ASCII string.
